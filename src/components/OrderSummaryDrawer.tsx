...
function useDialogFocusTrap(
  ...
  }, [enabled, returnFocusRef, containerRef]);
}

type OrderSummaryDrawerProps = {
  onClose: () => void;
  /** Element to restore focus when the drawer unmounts (e.g. the control that opened it). */
...
export default function OrderSummaryDrawer({
  ...
  useDialogFocusTrap(panelRef, returnFocusRef, true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div
        ...
        className="relative w-full max-w-2xl bg-white rounded-t-2xl animate-slide-up max-h-[80vh] flex flex-col"
      >
        <div className="p-6 pb-0">
          ...
          <div className="flex items-center justify-between mb-4">
            <h2
              id={titleId}
              className="font-heading font-bold text-lg text-warm-text"
            >
              Pesanan Kamu
            </h2>
            <button
              type="button"
              onClick={handleClose}
              aria-label="Tutup ringkasan pesanan"
              className="p-1.5 rounded-full hover:bg-gray-100"
            >
              <X className="w-5 h-5 text-warm-text-muted" aria-hidden />
            </button>
          </div>
        </div>
